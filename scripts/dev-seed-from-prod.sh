...
# Safely seed MeetsDev from production data without touching production files.
#
# Default behavior:
# - Reads from ~/Library/Application Support/Meets
# - Writes to ~/Library/Application Support/MeetsDev
# - Stages the current dev support dir first so unrelated dev-only files are preserved
# - Overlays production meets.db / sidecars and config.json into the staged temp directory
...
#   ./scripts/dev-seed-from-prod.sh --prod-dir "/path/to/Meets" --dev-dir "/path/to/MeetsDev"
#   ./scripts/dev-seed-from-prod.sh --dry-run

PROD_SUPPORT_DIR="${MEETS_PROD_SUPPORT_DIR:-$HOME/Library/Application Support/Meets}"
DEV_SUPPORT_DIR="${MEETS_DEV_SUPPORT_DIR:-$HOME/Library/Application Support/MeetsDev}"
COPY_CONFIG=1
DRY_RUN=0
FORCE=0
SWAPPED_OLD_DEV=0
...
running_processes() {
  ps ax -o pid=,comm= | awk '$2 == "Meets" || $2 == "MeetsDev" { print }'
}
...
PROD_DB="$PROD_SUPPORT_DIR/meets.db"
PROD_DB_SHM="$PROD_SUPPORT_DIR/meets.db-shm"
PROD_DB_WAL="$PROD_SUPPORT_DIR/meets.db-wal"
...
if [[ "$DRY_RUN" -eq 1 ]]; then
  STAGE_DIR="${DEV_PARENT}/meets-dev-seed-stage.${TIMESTAMP}"
else
  mkdir -p "$DEV_PARENT"
  STAGE_DIR="$(mktemp -d "${DEV_PARENT}/meets-dev-seed-stage.XXXXXX")"
fi
...
if [[ "$FORCE" -ne 1 ]]; then
  if running="$(running_processes)" && [[ -n "$running" ]]; then
    printf 'Refusing to seed while Meets or MeetsDev is running:\n%s\n' "$running" >&2
...
run_or_echo ditto "$PROD_DB" "$STAGE_DIR/meets.db"
if [[ -f "$PROD_DB_SHM" ]]; then
  run_or_echo ditto "$PROD_DB_SHM" "$STAGE_DIR/meets.db-shm"
fi
if [[ -f "$PROD_DB_WAL" ]]; then
  run_or_echo ditto "$PROD_DB_WAL" "$STAGE_DIR/meets.db-wal"
fi
...
if [[ "$DRY_RUN" -eq 0 ]]; then
  validate_sqlite_db "$STAGE_DIR/meets.db"
