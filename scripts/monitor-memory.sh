#!/usr/bin/env bash
# Purpose: Monitor Meets process memory health during meeting recording
# Created: 2026-05-26
#
...
#
# Output: live table + /tmp/meets-memory-YYYYMMDD-HHMMSS.tsv
#
# What it tracks:
...
# ── Find process ─────────────────────────────────────────────────────────────
PID=""
for NAME in MeetsDev Meets; do
    PID=$(pgrep -x "$NAME" 2>/dev/null | head -1)
    [[ -n "$PID" ]] && break
done

if [[ -z "$PID" ]]; then
    echo "Error: no Meets process found. Launch the app first." >&2
    exit 1
fi

APP_NAME=$(ps -p "$PID" -o comm= 2>/dev/null || echo "Meets")
LOG_FILE="/tmp/meets-memory-$(date +%Y%m%d-%H%M%S).tsv"
