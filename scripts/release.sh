...
#   - Production provisioning profile for com.meets.app when CloudKit
#     entitlements are enabled:
#       MEETS_PROVISIONING_PROFILE=/path/to/profile.provisionprofile
#   - Notary profile stored: xcrun notarytool store-credentials MeetsNotary
...
INSTALL_DIR="${MEETS_RELEASE_INSTALL_DIR:-$OUTPUT_DIR/install-root}"
APP_DIR="${MEETS_RELEASE_APP_DIR:-$INSTALL_DIR/Meets.app}"
...
HOMEBREW_CASK="${MEETS_HOMEBREW_CASK:-meets}"
SKIP_HOMEBREW_CHECK="${MEETS_SKIP_HOMEBREW_CHECK:-0}"
RELEASE_PR_MODE="${MEETS_RELEASE_PR_MODE:-0}"
VERIFY_DIR=""
MOUNT_POINT=""
HOSTED_MOUNT_POINT=""
...
if [[ "$RELEASE_PR_MODE" == "1" ]]; then
  echo "ERROR: MEETS_RELEASE_PR_MODE is no longer allowed to publish stable artifacts." >&2
...
if [[ -z "$PROVISIONING_PROFILE" ]]; then
  echo "ERROR: stable release builds require MEETS_PROVISIONING_PROFILE." >&2
  echo "Use the production provisioning profile for bundle ID com.meets.app with CloudKit container iCloud.com.meets.app." >&2
  exit 1
fi
...
DOWNLOAD_URL="https://github.com/GantisStorm/meets/releases/download/v${VERSION}/Meets-${VERSION}.dmg"
TAG="v${VERSION}"
RELEASE_TITLE="Meets ${VERSION}"
RELEASE_METADATA_BRANCH="${MEETS_RELEASE_METADATA_BRANCH:-codex/release-${VERSION}-appcast}"
DEFAULT_RELEASE_NOTES_FILE="$ROOT/docs/release-notes/${VERSION}.md"
RELEASE_NOTES_FILE="${MEETS_RELEASE_NOTES_FILE:-$DEFAULT_RELEASE_NOTES_FILE}"
RELEASE_NOTES=""
...
if [[ -n "${MEETS_RELEASE_NOTES_FILE:-}" && ! -f "$RELEASE_NOTES_FILE" ]]; then
...
### Install
1. Download \`Meets-${VERSION}.dmg\`
2. Open the DMG and drag Meets to Applications
...
verify_homebrew_autobump() {
  if [[ "$SKIP_HOMEBREW_CHECK" == "1" ]]; then
    echo "  Skipping official Homebrew cask livecheck because MEETS_SKIP_HOMEBREW_CHECK=1."
    HOMEBREW_CHECK_STATUS="skipped"
    return 0
  fi
...
resume_existing_release_publication() {
  ...
  VERIFY_DIR=$(mktemp -d)
  local hosted_dmg="$VERIFY_DIR/Meets-${VERSION}.dmg"
  ...
  meets_require_release_publication_ready \
...
RELEASE_BUILD_ENV=(
  ...
  MEETS_TELEMETRYDECK_APP_ID="$MEETS_TELEMETRYDECK_PRODUCTION_APP_ID"
  MEETS_TELEMETRY_CHANNEL="production"
  "${BUILD_ENV[@]}"
)
echo "  Bundle ID: com.meets.app"
...
APP_ZIP="$OUTPUT_DIR/Meets-app-${VERSION}.zip"
...
DMG_PATH="$OUTPUT_DIR/Meets-${VERSION}.dmg"
...
SPCTL_RESULT=$(spctl -a -vv "$MOUNT_POINT/Meets.app" 2>&1)
echo "  $SPCTL_RESULT"

STAPLE_RESULT=$(xcrun stapler validate "$MOUNT_POINT/Meets.app" 2>&1)
echo "  $STAPLE_RESULT"

"$ROOT/scripts/verify_signed_cloud_entitlements.sh" \
  "$MOUNT_POINT/Meets.app" \
  Production \
  com.meets.app \
  iCloud.com.meets.app \
  production
...
VERIFY_DIR=$(mktemp -d)
HOSTED_DMG="$VERIFY_DIR/Meets-${VERSION}.dmg"
...
HOSTED_APP_SPCTL_RESULT=$(spctl -a -vv "$HOSTED_MOUNT_POINT/Meets.app" 2>&1)
echo "  $HOSTED_APP_SPCTL_RESULT"

HOSTED_APP_STAPLE_RESULT=$(xcrun stapler validate "$HOSTED_MOUNT_POINT/Meets.app" 2>&1)
...
sed -i '' "s|https://github.com/GantisStorm/meets/releases/download/[^\"]*\\.dmg|$DOWNLOAD_URL|g" "$ROOT/docs/index.html"
sed -i '' "s|https://github.com/GantisStorm/meets/releases/download/.*\\.dmg|$DOWNLOAD_URL|g" "$ROOT/docs/llms.txt"
