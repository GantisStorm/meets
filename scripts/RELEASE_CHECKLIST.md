...
- [ ] Homebrew installed and updated enough to run post-release `brew livecheck --cask meets`

## Signing Profiles

Meets's default entitlements include CloudKit (`iCloud.com.meets.app`). Any cloud-entitled build must be signed with a provisioning profile whose app identifier matches the bundle ID and whose certificate matches the signing identity.

- [ ] Dev lane `MeetsDev` / `com.meets.dev`
  - Use profile: `../meets-ios/secrets/meetsmacosdevcloudkitcommeetsdev.provisionprofile`
  - Use identity: `Apple Development: Pranav Hari Guruvayurappan (59WTZW55XG)`
  - Use `MEETS_CODESIGN_TIMESTAMP=none`
  - `scripts/dev-test.sh --cloud-entitlements` auto-selects these values when that local profile exists

- [ ] Named dev lanes `com.meets.dev.a/b/c`
...
- [ ] Preprod `MeetsPreprod` / `com.meets.preprod`
  - Export `MEETS_PROVISIONING_PROFILE=/path/to/com.meets.preprod.profile`
  - Maintainer local profile: `../meets-ios/secrets/meetsmacospreproddeveloperidcloudkit.provisionprofile`
  - Use the Developer ID release identity unless intentionally overriding `MEETS_SIGN_IDENTITY`
  - Verify the embedded profile carries `iCloud.com.meets.app`

- [ ] Stable `Meets` / `com.meets.app`
  - Export `MEETS_PROVISIONING_PROFILE=/path/to/com.meets.app.profile`
  - Maintainer local profile: `../meets-ios/secrets/meetsmacosproductiondeveloperidcloudkit.provisionprofile`
...
- [ ] App installed to `/Applications/Meets.app`
- [ ] Verify signature: `codesign -dvvv /Applications/Meets.app 2>&1 | grep "Authority"`
  - Must show `Developer ID Application: Pranav Hari Guruvayurappan (58W55QJ567)`
- [ ] Verify effective entitlements:
  ```bash
  codesign -d --entitlements :- /Applications/Meets.app | plutil -p -
  ```
  - Must show CloudKit container `iCloud.com.meets.app`
...
- [ ] Verify the built app:
  ```bash
  scripts/verify_signed_cloud_entitlements.sh /Applications/Meets.app Production com.meets.app iCloud.com.meets.app production
  ```
- [ ] Mount the local DMG, then verify its app:
  ```bash
  scripts/verify_signed_cloud_entitlements.sh /Volumes/Meets/Meets.app Production com.meets.app iCloud.com.meets.app production
  ```
- [ ] Re-download and mount the GitHub release DMG, then verify its app with the same command:
  ```bash
  scripts/verify_signed_cloud_entitlements.sh /Volumes/Meets/Meets.app Production com.meets.app iCloud.com.meets.app production
  ```
- [ ] Stable and pre-production builds explicitly set `MEETS_ICLOUD_CONTAINER_ENVIRONMENT=Production`; omission must fail closed.
...
- [ ] **Step 1: Notarize the app bundle**
  ```bash
  ditto -c -k --keepParent /Applications/Meets.app Meets-app.zip
  xcrun notarytool submit Meets-app.zip --keychain-profile MeetsNotary --wait
  ```
  - Must show `status: Accepted`

- [ ] **Step 2: Staple the app bundle**
  ```bash
  xcrun stapler staple /Applications/Meets.app
  ```
  - Must show `The staple and validate action worked!`

- [ ] **Step 3: Create DMG from the STAPLED app**
  ```bash
  ./scripts/create_dmg.sh /Applications/Meets.app dist-release
  ```

- [ ] **Step 4: Notarize the DMG**
  ```bash
  xcrun notarytool submit dist-release/Meets-X.Y.Z.dmg --keychain-profile MeetsNotary --wait
  ```
  - Must show `status: Accepted`

- [ ] **Step 5: Staple the DMG**
  ```bash
  xcrun stapler staple dist-release/Meets-X.Y.Z.dmg
  ```
...
- [ ] **Mount the DMG and test the app inside it:**
  ```bash
  hdiutil attach dist-release/Meets-X.Y.Z.dmg
  spctl -a -vv "/Volumes/Meets/Meets.app"
  ```
...
- [ ] **Verify DMG has hardened runtime:**
  ```bash
  codesign -dvvv dist-release/Meets-X.Y.Z.dmg 2>&1 | grep "flags"
  ```
  - Must show `flags=0x10000(runtime)` — if missing, `create_dmg.sh` is broken

- [ ] **Install and launch:**
  ```bash
  cp -R "/Volumes/Meets/Meets.app" /Applications/Meets.app
  open /Applications/Meets.app
  ```
...
- [ ] **Verify version:**
  ```bash
  defaults read /Applications/Meets.app/Contents/Info.plist CFBundleShortVersionString
  ```
...
- [ ] **Re-download the hosted draft DMG and verify it matches the local artifact**
  ```bash
  gh release download vX.Y.Z -p "Meets-X.Y.Z.dmg" -D /tmp/meets-release-verify --clobber
  shasum -a 256 dist-release/Meets-X.Y.Z.dmg /tmp/meets-release-verify/Meets-X.Y.Z.dmg
  spctl -a -vv -t open --context context:primary-signature /tmp/meets-release-verify/Meets-X.Y.Z.dmg
  xcrun stapler validate /tmp/meets-release-verify/Meets-X.Y.Z.dmg
  ```
...
- [ ] **Verify Sparkle update flow metadata and artifact:**
  ```bash
  scripts/verify_update_flow.sh --version X.Y.Z --dmg dist-release/Meets-X.Y.Z.dmg --require-notarized
  ```
...
- [ ] **Verify the official Homebrew cask autobump path**
  - The canonical release flow runs `brew livecheck --cask meets` after the GitHub Release is published
  - It also runs `brew bump --cask --no-pull-requests meets` to confirm BrewTestBot owns update PRs
  - Homebrew's BrewTestBot owns official cask bump PRs for `meets` and opens them automatically about every 3 hours
  - If a PR does not appear after the next autobump cycle, investigate livecheck/autobump rather than using `brew bump-cask-pr`
  - Set `MEETS_SKIP_HOMEBREW_CHECK=1` only when intentionally skipping the release-time livecheck

- [ ] **Verify the official cask install path if the cask changed shape**
  ```bash
  brew install --cask meets
  ```
...
- [ ] **Verify the Homebrew CLI alias when the cask exposes the bundled binary**
  ...
  brew install meets
  command -v meets
  meets transcribe --help
  ```

## Post-release

- [ ] Verify GitHub Pages serves appcast: `curl -s https://gantisstorm.github.io/meets/appcast.xml | head -5`
