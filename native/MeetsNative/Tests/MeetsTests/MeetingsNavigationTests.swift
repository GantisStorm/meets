...
/// ledger sections, gutters, and follow-up counts those shelves are rendered
/// into.
...
struct MeetingBrowserShelfTests {
    ...
    @Test("a collapsed filtered family's count reports the matches it is hiding")
    func collapsedFilteredFamilyCountReportsHiddenMatches() throws {
        let entries = [
            entry(1, daysAgo: 2),
        ...
        // and all four sit behind the fold: thread order is untouched, and the
        // count reports the matches it is holding back rather than reordering
        // the thread to reach them.
        #expect(shelf.root.matchesFilter)
        #expect(shelf.nodes.map(\.id) == [1, 2, 3, 4, 5])
        #expect(shelf.descendants.map(\.matchesFilter) == [false, true, false, true])

        #expect(MeetingBrowserLogic.followUpCountLabel(
    ...
    @Test("the count names the follow-up total and, folded, how much of it is in range")
    func followUpCountLabelNamesFollowUpsAndRangeMatches() {
        #expect(MeetingBrowserLogic.followUpCountLabel(
            descendantCount: 1,
            hiddenMatchCount: 1,
            annotatesMatches: false,
        ...
        // Expanded, the follow-ups are on screen under the count, so the range
