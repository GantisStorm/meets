...
struct Qwen3VendorTests {
    ...
    func defaultCacheMatchesManagedPlan() {
        ...
        let cache = MeetsQwen3AsrModels.defaultCacheDirectory(variant: .int8)
...
struct Qwen3LanguageTests {
    ...
    func languageInitParsesCodesAndNames() {
        #expect(MeetsQwen3AsrConfig.Language(from: "en") == .english)
        #expect(MeetsQwen3AsrConfig.Language(from: "English") == .english)
        #expect(MeetsQwen3AsrConfig.Language(from: "ENGLISH") == .english)
        #expect(MeetsQwen3AsrConfig.Language(from: "hi") == .hindi)
    }

    @Test("Language init rejects unknown input")
    func languageInitRejectsUnknownInput() {
        #expect(MeetsQwen3AsrConfig.Language(from: "eng") == nil)
        #expect(MeetsQwen3AsrConfig.Language(from: "") == nil)
        #expect(MeetsQwen3AsrConfig.Language(from: "chinese (simplified)") == nil)
    }
}
...
struct Qwen3LanguageSelectionTests {
    ...
    func exposesCodesAndLabels() {
        ...
        #expect(Qwen3AsrLanguage.allCases.count == MeetsQwen3AsrConfig.Language.allCases.count + 1)
    }
