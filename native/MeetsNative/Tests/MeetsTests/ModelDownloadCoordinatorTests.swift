...
struct ModelDownloadCoordinatorTests {
    ...
    func meetsMirrorManifestRejectsUntrustedOrigin() async throws {
        ...
        await #expect(throws: MeetsModelMirrorManifestError.self) {
            try await resolver.resolve(
                modelID: "acme/asr",
                mirror: MeetsModelMirror(manifestURL: try #require(URL(string: "https://example.com/models/acme/asr/mirror-v1/manifest.json")))
            )
        }
    }
    ...
    func meetsMirrorManifestRejectsUnsafeEntries() async throws {
        ...
        for testCase in cases {
            ...
            await #expect(throws: MeetsModelMirrorManifestError.self) {
                try await resolver.resolve(
                    modelID: "acme/asr",
                    mirror: MeetsModelMirror(manifestURL: manifestURL)
                )
            }
    ...
    func managedDownloadPrefersMeetsMirror() async throws {
        ...
        let directory = try await ManagedASRModelDownloader.downloadIfNeeded(
            ...
            mirrorResolver: MeetsModelMirrorManifestResolver(configuration: makeSessionConfiguration()),
    ...
    func managedDownloadFallsBackWhenMeetsMirrorTransferFails() async throws {
        let root = try makeTemporaryDirectory()
        defer { try? FileManager.default.removeItem(at: root) }
