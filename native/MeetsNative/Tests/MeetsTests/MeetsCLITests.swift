...
import Testing
import MeetsCore
@testable import MeetsCLI

@Suite("MeetsCLI", .serialized)
struct MeetsCLITests {
    @Test("spec exposes the agent-facing command set")
    func specPayloadIncludesCommands() {
        let payload = MeetsCLI.specPayload()
    ...
    func loadCustomWordsRejectsMissingFile() {
        ...
        #expect(throws: Error.self) {
            _ = try MeetsAudioTranscriptionPipeline.loadCustomWords(from: url)
        }
    }
