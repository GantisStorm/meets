...
struct OpenRouterAuthTests {
    ...
    func authorizationURL() throws {
        let auth = makeAuth()
        let callbackURL = URL(string: "http://localhost:54321/meets/openrouter/oauth/secret")!
        let url = try #require(
    ...
    func callbackValidation() {
        let auth = makeAuth()
        let path = "/meets/openrouter/oauth/secret"
        ...
        #expect(
            auth.parseCallbackRequest(
                "GET /meets/openrouter/oauth/wrong?code=stolen HTTP/1.1\r\n\r\n",
                expectedPath: path
            ) == .unrelated
        )
        #expect(
