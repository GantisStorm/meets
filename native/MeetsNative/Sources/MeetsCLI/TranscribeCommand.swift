...
struct TranscribeCommand: AsyncParsableCommand {
    ...
    mutating func validate() throws {
        ...
        guard MeetsAudioFilePreparer.isSupportedFileURL(url) else {
    ...
    func run() async throws {
        ...
        let result = try await pipeline.run(
            request: MeetsAudioTranscriptionRequest(
                sourceURL: sourceURL,
                model: model,
                title: title,
                summarize: summarize,
...
extension TranscribeJSONPayload {
    init(_ result: MeetsAudioTranscriptionResult) {
        self.init(
...
struct MeetsAudioTranscriptionRequest {
    let sourceURL: URL
...
struct MeetsAudioTranscriptionResult {
    let title: String
    let transcript: String
    let summary: String?
    let durationSeconds: Double
...
struct MeetsAudioTranscriptionPipeline {
    ...
    init(
        audioPreparer: AudioPreparing = MeetsAudioFilePreparer(),
        transcriber: AudioTranscribing = RoutingAudioTranscriber(),
        summarizer: MeetingSummarizing = ConfiguredCLIMeetingSummarizer(),
        dataChangePoster: @escaping () -> Void = MeetsNotifications.postDataDidChange
    ) {
    ...
    func run(request: MeetsAudioTranscriptionRequest, context: CLIContext) async throws -> MeetsAudioTranscriptionResult {
        ...
        return MeetsAudioTranscriptionResult(
            title: title,
...
struct MeetsAudioFilePreparer: AudioPreparing {
...
/// Wraps the vendored `MeetsQwen3AsrManager` directly — a thin wrapper, same shape as
...
actor Qwen3AsrCLITranscriber: AudioTranscribing {
    ...
    private func transcribeOnSupportedOS(wavURL: URL, progress: @escaping (String) -> Void) async throws -> HeadlessTranscription {
        ...
        guard let qwen3Manager = manager as? MeetsQwen3AsrManager else {
            throw CLIError.invalidInput("Qwen3 ASR model was not loaded.", fix: "Run the command again after the model finishes downloading.")
        }
