...
enum RecordingWaveformCacheFiles {
    ...
    static func removeLegacyJSONWaveformCaches(
        ...
        if removed > 0 {
            logger?("[meets] cleaned up \(removed) legacy waveform JSON cache file\(removed == 1 ? "" : "s")")
        }
    ...
    static func sweepOrphanedCachedWaveforms(
        ...
        if removed > 0 {
            logger?("[meets] cleaned up \(removed) orphaned waveform cache file\(removed == 1 ? "" : "s")")
        }
        return .completed(removed: removed)
    }
...
struct MeetingRecordingPlayerView: View {
    ...
    var body: some View {
        ...
        .background(MeetsTheme.backgroundRaised.opacity(0.74))
        ...
        GeometryReader { proxy in
            Canvas { context, size in
                ...
                for index in 0..<barCount {
                    ...
                    let color = x <= playedX ? MeetsTheme.accent : MeetsTheme.textTertiary.opacity(0.24)
                    context.fill(
