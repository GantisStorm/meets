...
enum DictationContextCapture {
    ...
    static func capture(allowTitleFallback: Bool = true) -> DictationContext {
        ...
        fputs("[meets] meeting context: app=\(appName) docContext=\(docContext.count) chars url=\(url ?? "none")\n", stderr)
    ...
    /// Formats for the LLM prompt. Compact, high-signal.
...
enum ScreenContextCapture {
    ...
    private static func captureFrontmostWindow(
        ...
        guard let focusedWindow = DictationContextCapture.focusedWindowSnapshot(
            ...
            fputs("[meets] screen context: focused window is not available for \(appName)\n", stderr)
            return nil
        }
        ...
        guard let windowID = focusedWindowID(
            ...
            fputs("[meets] screen context: focused window could not be bound for \(appName)\n", stderr)
            return nil
        }
        ...
        guard let image = CGWindowListCreateImage(
            ...
            fputs("[meets] screen context: screenshot capture failed\n", stderr)
            return nil
        }

        do {
            let text = try await ocrImage(image)
            fputs("[meets] screen context: captured \(text.count) \(logLabel) chars from \(appName)\n", stderr)
            ...
            fputs("[meets] screen context: \(logLabel) failed: \(error)\n", stderr)
            return nil
        }
    }
