...
final class StreamingMicRecorder: StreamingDictationRecording, StreamingDictationLatencyReporting, PausableStreamingDictationRecording {
    ...
    private let failureCallbackQueue = DispatchQueue(label: "com.meets.streaming-mic-recorder-failures")
    ...
    private let configurationChangeQueue = DispatchQueue(label: "com.meets.streaming-mic-recorder-config-change")
    ...
    init(
        directoryName: String = "meets-meeting-mic",
    ...
    private func prepareLocked() throws {
        ...
        if recoversFromInputConfigurationChanges {
            if let preferredInputDeviceID,
               let error = MeetsAudioGraphSetInputDevice(engine, preferredInputDeviceID) {
                throw error
            }
        } else {
        ...
        if recoversFromInputConfigurationChanges {
            if let error = MeetsAudioGraphPrepareEngine(engine) { throw error }
        } else {
            engine.prepare()
        }
    ...
    private func startEngineWithTapLocked(recordingID: UUID) throws {
        ...
        if recoversFromInputConfigurationChanges {
            if let tapError = MeetsAudioGraphInstallInputTap(engine, 0, Self.bufferSize, nil, tapBlock) {
                throw tapError
            }
        } else {
        ...
        if recoversFromInputConfigurationChanges {
            if let error = MeetsAudioGraphStartEngine(engine) { throw error }
        } else {
            try engine.start()
        }
    ...
    private func inputFormatLocked() throws -> AVAudioFormat {
        if recoversFromInputConfigurationChanges {
            let state = MeetsAudioGraphReadInputState(engine)
    ...
    private func removeTapIfNeeded() {
        ...
        if recoversFromInputConfigurationChanges {
            _ = MeetsAudioGraphRemoveInputTap(engine, 0)
        } else {
    ...
    private func stopEngineSafely() {
        if recoversFromInputConfigurationChanges {
            _ = MeetsAudioGraphStopEngine(engine)
        } else {
            engine.stop()
        }
    }
