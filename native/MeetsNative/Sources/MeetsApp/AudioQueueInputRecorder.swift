...
final class AudioQueueInputRecorder: StreamingDictationRecording, StreamingDictationLatencyReporting, PausableStreamingDictationRecording {
    ...
    private let processingQueue = DispatchQueue(label: "com.meets.audio-queue-input-recorder-processing")
    private let failureCallbackQueue = DispatchQueue(label: "com.meets.audio-queue-input-recorder-failures")
    ...
    init(directoryName: String = "meets-native-dictation", native: NativeOperations = NativeOperations()) {
        self.native = native
