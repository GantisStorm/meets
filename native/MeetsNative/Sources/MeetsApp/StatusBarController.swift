...
final class StatusBarController: NSObject, NSMenuDelegate {
    ...
    init(controller: MeetsController, runtime: RuntimePaths) {
    ...
    func updateMenuBarTitle() {
        ...
        statusItem.button?.attributedTitle = MenuBarIconRenderer.statusTitle(
            hotkey: controller.config.meetingRecordingHotkey,
    ...
    private func rebuildMenu() {
        ...
        menu.addItem(actionItem(title: "Open \(AppIdentity.displayName)", action: #selector(MeetsController.openHistoryWindow as (MeetsController) -> () -> Void)))
        if controller.isMeetingRecording() {
            ...
            menu.addItem(actionItem(title: pauseTitle, action: #selector(MeetsController.toggleMeetingRecordingPause)))
            menu.addItem(actionItem(title: "Stop Meeting Recording", action: #selector(MeetsController.toggleMeetingRecording)))
            menu.addItem(actionItem(title: "Discard Meeting Recording...", action: #selector(MeetsController.discardMeetingWithConfirmation)))
        } else {
            menu.addItem(actionItem(
                title: "Start Meeting Recording",
                action: #selector(MeetsController.startMeetingRecordingFromMenuBar)
            ))
        }
        menu.addItem(.separator())
        ...
        for option in usableBackends {
            ...
            let item = NSMenuItem(
                ...
                action: #selector(MeetsController.selectMeetingSummaryBackendFromMenu(_:)),
                keyEquivalent: ""
            )
        ...
        menu.addItem(.separator())
        menu.addItem(actionItem(title: "Settings…", action: #selector(MeetsController.openSettingsTab)))
        menu.addItem(checkForUpdatesItem())
        menu.addItem(.separator())
        menu.addItem(actionItem(title: "Quit", action: #selector(MeetsController.quitApp)))
    }
    ...
    private func addUpcomingEventGroup(
        ...
        for event in displayedEvents {
            ...
            let item = NSMenuItem(
                title: "\(event.title)\n\(timeStr)",
                action: #selector(MeetsController.startMeetingFromCalendarMenuItem(_:)),
                keyEquivalent: ""
            )
            item.target = controller
    ...
    private func checkForUpdatesItem() -> NSMenuItem {
        let item = NSMenuItem(
            ...
            action: #selector(MeetsController.checkForUpdates),
            keyEquivalent: ""
        )
        item.target = controller
