...
private actor MeetingDetectionService {
    private static let logger = Logger(subsystem: "com.meets.native", category: "MeetingDetection")
...
actor AudioAttributionService {
    ...
    func activeOutputProcesses(refresh: Bool) -> AudioAttributionResult {
        guard refresh else {
            return AudioAttributionResult(processes: cachedOutputProcesses, duration: 0, startedRefresh: false)
        }

        let start = Date()
        let processes = AudioProcessAttributionCollector().activeOutputProcesses()
