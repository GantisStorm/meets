...
struct MeetingDetailView: View {
    ...
    private func standardHeader(_ meeting: MeetingRecord) -> some View {
        ...
        return VStack(alignment: .leading, spacing: MeetsTheme.spacing16) {
            if let onBack {
    ...
    private func compactRecordingControls(for meeting: MeetingRecord) -> some View {
        if meeting.status == .recording, !isPreparingThisMeeting(meeting) {
            HStack(spacing: 5) {
                Circle()
                    .fill(appState.isMeetingRecordingPaused ? MeetsTheme.transcribing : MeetsTheme.recording)
    ...
    private func statusChip(for meeting: MeetingRecord) -> some View {
        ...
        let color = isPreparing || isPaused ? MeetsTheme.transcribing : meeting.status.displayColor
        HStack(spacing: 6) {
            Circle()
                .fill(color)
                .frame(width: 7, height: 7)
    ...
    private var pauseResumeRecordingButton: some View {
        ...
        return Button {
            ...
            .foregroundStyle(isPaused ? MeetsTheme.accentContent : MeetsTheme.textPrimary)
            .padding(.horizontal, MeetsTheme.spacing12)
            .padding(.vertical, 7)
            .background(isPaused ? MeetsTheme.accent : MeetsTheme.surfacePrimary)
            .clipShape(RoundedRectangle(cornerRadius: MeetsTheme.cornerSmall))
            .overlay(
                RoundedRectangle(cornerRadius: MeetsTheme.cornerSmall)
                    .strokeBorder(isPaused ? MeetsTheme.accent.opacity(0.35) : MeetsTheme.surfaceBorder, lineWidth: 1)
            )
        }
        .buttonStyle(.plain)
    ...
    private func folderPill(for meeting: MeetingRecord) -> some View {
        ...
        Button {
            ...
            .foregroundStyle(hasFolder ? MeetsTheme.accent : MeetsTheme.textSecondary)
            .padding(.horizontal, MeetsTheme.spacing8)
            .frame(height: MeetingHeaderLayout.contextControlHeight)
            .background(hasFolder ? MeetsTheme.accentSubtle : MeetsTheme.backgroundRaised)
            .clipShape(RoundedRectangle(cornerRadius: MeetsTheme.cornerSmall))
            .overlay(
                RoundedRectangle(cornerRadius: MeetsTheme.cornerSmall)
                    .strokeBorder(hasFolder ? Color.clear : MeetsTheme.surfaceBorder, lineWidth: 1)
            )
        }
        .buttonStyle(.plain)
    ...
    private func eventPill(for meeting: MeetingRecord) -> some View {
        ...
        Button {
            ...
            .foregroundStyle(hasLinks ? MeetsTheme.accent : MeetsTheme.textSecondary)
            .padding(.horizontal, MeetsTheme.spacing8)
            .frame(height: MeetingHeaderLayout.contextControlHeight)
            .background(hasLinks ? MeetsTheme.accentSubtle : MeetsTheme.backgroundRaised)
            .clipShape(RoundedRectangle(cornerRadius: MeetsTheme.cornerSmall))
            .overlay(
                RoundedRectangle(cornerRadius: MeetsTheme.cornerSmall)
                    .strokeBorder(hasLinks ? Color.clear : MeetsTheme.surfaceBorder, lineWidth: 1)
            )
        }
        .buttonStyle(.plain)
        ...
        Button(action: action) {
            HStack(spacing: 8) {
                Image(systemName: isLinked ? "calendar.circle.fill" : "calendar.circle")
                    .font(.system(size: 14))
                    .foregroundStyle(isLinked ? MeetsTheme.accent : MeetsTheme.textTertiary)
    ...
    private var transcriptCTA: some View {
        ...
        .background(MeetsTheme.accent.opacity(0.08))
        .clipShape(RoundedRectangle(cornerRadius: MeetsTheme.cornerSmall))
    }
