...
enum SoundController {
    ...
    static func playMaraudersMapClip(id: String, customPath: String?, onFinished: (() -> Void)? = nil) {
        ...
        guard let url, FileManager.default.fileExists(atPath: url.path) else {
            fputs("[meets] Marauder's Map audio clip not found: \(id)\n", stderr)
            return
        }
...
private enum SystemSoundPlayer {
    private static let queue = DispatchQueue(label: "com.meets.system-sound-player", qos: .userInitiated)
    ...
    private static func loadSoundID(at url: URL, cacheKey: String) -> SystemSoundID? {
        ...
        guard status == kAudioServicesNoError else {
            fputs("[meets] failed to load sound \(url.lastPathComponent): \(status)\n", stderr)
            return nil
        }
