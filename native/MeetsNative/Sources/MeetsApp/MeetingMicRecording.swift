...
final class RouteAwareMeetingMicRecorder: MeetingMicRecording {
    ...
    init(
        ...
        lifecycleQueue: DispatchQueue = DispatchQueue(label: "com.meets.route-aware-meeting-mic-recorder-lifecycle"),
        handoffWorkerQueue: DispatchQueue = DispatchQueue(
            label: "com.meets.route-aware-meeting-mic-recorder-handoff",
            attributes: .concurrent
        ),
        cleanupQueue: DispatchQueue = DispatchQueue(
            label: "com.meets.route-aware-meeting-mic-recorder-cleanup",
            attributes: .concurrent
        ),
    ...
    private func beginHandoffIfNeeded(
        ...
        lock.withLock {
            $0.pending = candidate
            $0.startingIDs.insert(candidate.id)
        }
    ...
    private static func makeSystemDefaultRecorder() -> MeetingMicRecording {
        StreamingMeetingMicRecorderAdapter(
            recorder: StreamingMicRecorder(
                directoryName: "meets-meeting-mic",
    ...
    private static func makeAppScopedRecorder() -> MeetingMicRecording {
        StreamingMeetingMicRecorderAdapter(
            recorder: FallbackStreamingDictationRecorder(
                primary: AudioQueueInputRecorder(directoryName: "meets-meeting-mic-audioqueue"),
                fallback: StreamingMicRecorder(
                    directoryName: "meets-meeting-mic-app-scoped-fallback",
                    recoversFromInputConfigurationChanges: true,
                    observesInputConfigurationChanges: false
                )
            ),
