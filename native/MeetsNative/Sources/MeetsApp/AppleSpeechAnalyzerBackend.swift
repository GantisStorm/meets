...
actor AppleSpeechAnalyzerTranscriber {
    ...
    private func performPrepare(
        ...
        fputs("[meets] Apple Speech ready for \(locale.identifier(.bcp47))\n", stderr)
        return locale
    }
    ...
    func transcribe(wavURL: URL, requestedLocale: Locale = .current) async throws -> SpeechTranscriptionResult {
        ...
        fputs(
            "[meets] Apple Speech completed \(result.text.count) characters in \(elapsedText)s (locale \(locale.identifier(.bcp47)))\n",
            stderr
        )
        return result
    }
