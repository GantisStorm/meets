...
private struct FloatingMeetingTranscriptPanelView: View {
    ...
    private var header: some View {
        HStack(spacing: MeetsTheme.spacing8) {
            ...
            Spacer()
            Circle()
                .fill(model.isPaused ? MeetsTheme.textTertiary : MeetsTheme.success)
            ...
            Button(action: copyTranscript) {
                Image(systemName: model.didCopy ? "checkmark" : "doc.on.doc")
                    .font(.system(size: 12, weight: .semibold))
                    .foregroundStyle(model.didCopy ? MeetsTheme.success : MeetsTheme.textSecondary)
                    .frame(width: 24, height: 24)
                    .contentShape(Rectangle())
            }
            .buttonStyle(.plain)
    ...
    private var transcript: some View {
        ScrollView {
            LiveTranscriptFeedView(
                ...
                partialYou: partialYou,
                partialOthers: partialOthers,
                horizontalPadding: MeetsTheme.spacing12,
                topPadding: MeetsTheme.spacing8,
