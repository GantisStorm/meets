...
struct CalendarEventDetailView: View {
                    ...
                    HStack(spacing: MeetsTheme.spacing8) {
                        ...
                        if meeting.title != pendingTitleText {
                            Button {
                                ...
                                .clipShape(Capsule())
                                .overlay(
                                    Capsule().strokeBorder(MeetsTheme.surfaceBorder, lineWidth: 1)
                                )
                            }
                            .buttonStyle(.plain)
    ...
    private var calendarLine: some View {
        HStack(spacing: 6) {
            Circle()
                .fill(calendarColor ?? MeetsTheme.accent)
    ...
    private var cancelledBanner: some View {
        ...
        .padding(MeetsTheme.spacing12)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(MeetsTheme.transcribing.opacity(0.10))
        .clipShape(RoundedRectangle(cornerRadius: MeetsTheme.cornerMedium))
        .overlay(
            RoundedRectangle(cornerRadius: MeetsTheme.cornerMedium)
                .strokeBorder(MeetsTheme.transcribing.opacity(0.35), lineWidth: 1)
        )
    }
            ...
            VStack(alignment: .leading, spacing: MeetsTheme.spacing8) {
                ...
                if let joinURL = event.meetingURL {
                    Button {
                        ...
                        .overlay(
                            RoundedRectangle(cornerRadius: MeetsTheme.cornerSmall)
                                .strokeBorder(MeetsTheme.accent.opacity(0.4), lineWidth: 1)
                        )
                    }
                    .buttonStyle(.plain)
                    .disabled(busy)
    ...
    private var recordingZone: some View {
        ...
        .padding(MeetsTheme.spacing12)
        .frame(maxWidth: .infinity, alignment: .leading)
        .background(MeetsTheme.recording.opacity(0.08))
        .clipShape(RoundedRectangle(cornerRadius: MeetsTheme.cornerMedium))
        .overlay(
            RoundedRectangle(cornerRadius: MeetsTheme.cornerMedium)
                .strokeBorder(MeetsTheme.recording.opacity(0.35), lineWidth: 1)
        )
    }
    ...
    private func openMeetingButton(_ meeting: MeetingRecord, label: String, prominent: Bool = false) -> some View {
        Button {
            ...
            .foregroundStyle(prominent ? MeetsTheme.accentContent : MeetsTheme.textPrimary)
            .padding(.horizontal, MeetsTheme.spacing12)
            .padding(.vertical, 7)
            .background(prominent ? MeetsTheme.accent : MeetsTheme.surfacePrimary)
            .clipShape(RoundedRectangle(cornerRadius: MeetsTheme.cornerSmall))
            .overlay(
                RoundedRectangle(cornerRadius: MeetsTheme.cornerSmall)
                    .strokeBorder(prominent ? MeetsTheme.accent.opacity(0.35) : MeetsTheme.surfaceBorder, lineWidth: 1)
            )
        }
        .buttonStyle(.plain)
        .fixedSize()
    ...
    private func summaryStatusColor(for meeting: MeetingRecord) -> Color {
        ...
        case .noteOnly, .completed:
            return hasSummary ? MeetsTheme.success : MeetsTheme.textTertiary
        }
    }
