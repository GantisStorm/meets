...
enum Qwen3AsrLanguage: Hashable, Sendable {
    case auto
    case pinned(MeetsQwen3AsrConfig.Language)

    static let defaultLanguage: Self = .auto

    static var allCases: [Qwen3AsrLanguage] {
        [.auto] + MeetsQwen3AsrConfig.Language.allCases.map(Qwen3AsrLanguage.pinned)
    }
    ...
    static func resolved(_ rawValue: String?) -> Self {
        ...
        if let language = MeetsQwen3AsrConfig.Language(rawValue: normalized) {
            return .pinned(language)
        }
        if let language = MeetsQwen3AsrConfig.Language(from: normalized) {
            return .pinned(language)
        }
        return defaultLanguage
    }
...
/// Scope of the OpenRouter model catalog consumed by Meeting Summaries.
/// Only text-output models are surfaced; the transcription catalog was
/// removed with the dictation strip.
...
/// Loads the OpenRouter model catalog for the Meeting Summaries free-model
/// picker. Request/parse logic mirrors the retired transcription client,
/// restricted to the `.text` scope.
