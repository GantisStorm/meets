...
struct CalendarPageView: View {
                ...
                VStack(alignment: .leading, spacing: MeetsTheme.spacing16) {
                    ...
                    HStack(spacing: MeetsTheme.spacing12) {
                        Button {
                            ...
                            .foregroundStyle(MeetsTheme.backgroundBase)
    ...
    private func monthDayCell(_ cell: CalendarPageDayCell) -> some View {
            ...
            Button(action: openDay) {
                Text(dayNumber)
                    ...
                    .foregroundStyle(isSelected
                        ? MeetsTheme.accentContent
                        : (cell.isCurrentMonth ? MeetsTheme.textPrimary : MeetsTheme.textTertiary))
                    ...
                    .overlay(
                        Circle()
                            .strokeBorder(isToday ? MeetsTheme.accent : Color.clear, lineWidth: 1.5)
                    )
            }
            .buttonStyle(.plain)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: MeetsTheme.cornerSmall)
                .strokeBorder(cell.isCurrentMonth ? MeetsTheme.surfaceBorder.opacity(0.5) : Color.clear, lineWidth: 1)
        )
    ...
    private func backgroundFill(isSelected: Bool, isToday: Bool, isCurrentMonth: Bool) -> Color {
        if isSelected {
            return MeetsTheme.surfaceSelected.opacity(0.5)
        }
        if isCurrentMonth {
            return MeetsTheme.backgroundRaised.opacity(0.55)
        }
        return .clear
    }

    private func monthChip(_ event: UnifiedCalendarEvent) -> some View {
        let color = calendarColor(for: event) ?? MeetsTheme.accent
        ...
        return HStack(spacing: 3) {
            ...
            Text(event.title)
                ...
                .lineLimit(1)
                .truncationMode(.tail)
                .strikethrough(cancelled, color: MeetsTheme.textSecondary)
                .foregroundStyle(cancelled ? MeetsTheme.textTertiary : MeetsTheme.textPrimary.opacity(0.9))
    ...
    private var listView: some View {
        ...
        return ScrollViewReader { proxy in
            ScrollView {
                LazyVStack(alignment: .leading, spacing: MeetsTheme.spacing20) {
                    if sections.isEmpty {
                        listEmptyState
                    } else {
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack(spacing: 8) {
                Text(section.title)
                    ...
                    .foregroundStyle(section.isToday ? MeetsTheme.accent : MeetsTheme.textPrimary)
    ...
    private func eventRow(
        ...
        let row = HStack(alignment: .top, spacing: 8) {
            Text(CalendarPageLogic.startTimeFormatter.string(from: event.startDate))
                .font(.system(size: 12, weight: .medium))
                .foregroundStyle(isDimmed ? MeetsTheme.textTertiary : MeetsTheme.textSecondary)
                .monospacedDigit()
            ...
            VStack(alignment: .leading, spacing: 2) {
                Text(event.title)
                    ...
                    .foregroundStyle(isDimmed ? MeetsTheme.textTertiary : MeetsTheme.textPrimary)
                    .strikethrough(event.isCancelled || event.isDeclined, color: MeetsTheme.textTertiary)
                    .lineLimit(1)

                HStack(spacing: 6) {
                    Text(CalendarPageLogic.rangeFormatter.timeRange(for: event))
                        .font(.system(size: 11))
                        .foregroundStyle(isDimmed ? MeetsTheme.textTertiary.opacity(0.8) : MeetsTheme.textSecondary)
                        .lineLimit(1)
        ...
        .overlay(
            RoundedRectangle(cornerRadius: MeetsTheme.cornerMedium)
                .strokeBorder(linkage.state == .now ? MeetsTheme.accent.opacity(0.45) : MeetsTheme.surfaceBorder, lineWidth: 1)
        )
        .contentShape(Rectangle())
    ...
    private func recordButton(for event: UnifiedCalendarEvent, prominent: Bool, busy: Bool) -> some View {
        Button {
            ...
            .foregroundStyle(prominent ? MeetsTheme.backgroundBase : MeetsTheme.accent)
            .padding(.horizontal, 8)
            .padding(.vertical, 4)
            .background(prominent ? MeetsTheme.accent : MeetsTheme.accent.opacity(0.12))
            .clipShape(Capsule())
        }
        .buttonStyle(.plain)
        .disabled(busy)
    ...
    private var recordingChip: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(MeetsTheme.recording.opacity(0.12))
        .clipShape(Capsule())
    ...
    private var processingChip: some View {
        ...
        .padding(.horizontal, 8)
        .padding(.vertical, 4)
        .background(MeetsTheme.transcribing.opacity(0.12))
        .clipShape(Capsule())
        .fixedSize()
        .accessibilityElement(children: .ignore)
