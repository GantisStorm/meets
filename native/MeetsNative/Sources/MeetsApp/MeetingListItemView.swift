...
struct MeetingListItemView: View {
    ...
    private func relationshipIcon(_ systemName: String, help: String) -> some View {
        Image(systemName: systemName)
            ...
            .foregroundStyle(MeetsTheme.accent.opacity(0.9))
    ...
    private func sourceBadge(icon: String, label: String, help: String) -> some View {
        ...
        .foregroundStyle(isImportedAudio ? MeetsTheme.accent : MeetsTheme.textSecondary)
        .padding(.horizontal, 6)
        .padding(.vertical, 2)
        .background((isImportedAudio ? MeetsTheme.accent : MeetsTheme.textSecondary).opacity(0.12))
        .clipShape(Capsule())
        .help(help)
        .accessibilityLabel(help)
    }
