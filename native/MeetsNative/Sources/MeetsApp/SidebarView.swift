    ...
    var body: some View {
        Button(action: action) {
            Image(systemName: "sidebar.left")
                ...
                .background(MeetsTheme.backgroundRaised.opacity(0.72))
                .clipShape(Circle())
                .overlay(Circle().strokeBorder(MeetsTheme.surfaceBorder, lineWidth: 1))
        }
        .buttonStyle(.plain)
...
struct SidebarView: View {
    ...
    private var updateCTAForeground: Color {
        ...
        let defaultAccentHex = colorScheme == .dark
            ? MeetsTheme.defaultAccentDarkHex
            : MeetsTheme.defaultAccentLightHex
    ...
    private var collapsedSidebar: some View {
        ...
        .background(MeetsTheme.backgroundDeep)
    }
    ...
    private func collapsedItem(tab: DashboardTab, icon: String, label: String) -> some View {
        Button {
            ...
            Image(systemName: icon)
                .font(.system(size: 16, weight: .medium))
                .foregroundStyle(appState.selectedTab == tab ? MeetsTheme.accent : MeetsTheme.textSecondary)
                .frame(width: 40, height: 36)
                .background(appState.selectedTab == tab ? MeetsTheme.surfacePrimary : Color.clear)
    ...
    private var sidebarHeader: some View {
        HStack(alignment: .top) {
            VStack(alignment: .leading, spacing: MeetsTheme.spacing4) {
                MeetsWordmark(size: 26, color: MeetsTheme.textPrimary)
                if !userName.isEmpty {
                    Text("Hi, \(userName)")
        ...
        .padding(.bottom, MeetsTheme.spacing20)
    }
            ...
            HStack(spacing: MeetsTheme.spacing12) {
                Button {
                    ...
                    HStack(spacing: MeetsTheme.spacing12) {
                        ...
                        Text("Meetings")
                            ...
                            .foregroundStyle(isSelected ? MeetsTheme.textPrimary : MeetsTheme.textSecondary)
                        Spacer(minLength: 0)
                    }
                ...
                Button {
                    ...
                    Image(systemName: meetingsExpanded ? "chevron.down" : "chevron.right")
                        ...
                        .foregroundStyle(isSelected ? MeetsTheme.textSecondary : MeetsTheme.textTertiary)
        ...
        Button {
            ...
            HStack(spacing: MeetsTheme.spacing12) {
                ...
                if let updateCTA {
                    ...
                    .clipShape(Capsule())
                    .shadow(color: MeetsTheme.accent.opacity(0.35), radius: 8, x: 0, y: 2)
        ...
        HStack(spacing: 2) {
            Button {
                ...
                Image(systemName: "sun.max.fill")
                    ...
                    .foregroundStyle(!isDark ? MeetsTheme.accent : MeetsTheme.textTertiary)
                    .frame(width: 28, height: 22)
                    .contentShape(Rectangle())
                    .background(
                        RoundedRectangle(cornerRadius: 4)
                            .fill(!isDark ? MeetsTheme.surfaceSelected : Color.clear)
                    )
            }
            .buttonStyle(.plain)
    ...
    private func meetingFilterRow(
        ...
        HStack(spacing: MeetsTheme.spacing12) {
            Image(systemName: icon)
                .font(.system(size: 14, weight: .medium))
                .foregroundStyle(isSelected ? MeetsTheme.accent : MeetsTheme.textTertiary)
                .frame(width: sidebarIconColumnWidth)
            Text(label)
                .font(MeetsTheme.callout())
                .foregroundStyle(isSelected ? MeetsTheme.textPrimary : MeetsTheme.textSecondary)
        ...
        .background(
            RoundedRectangle(cornerRadius: MeetsTheme.cornerSmall)
                .fill(isSelected ? MeetsTheme.surfaceSelected.opacity(0.6) : Color.clear)
        )
        .contentShape(Rectangle())
    ...
    private func folderRenameField(folder: MeetingFolder) -> some View {
        ...
        .background(
            RoundedRectangle(cornerRadius: MeetsTheme.cornerSmall)
                .fill(MeetsTheme.surfaceSelected.opacity(0.6))
        )
    }
