...
public enum ManagedASRModelPlans {
    ...
    private static func whisperKitMirror(fullName: String) -> MeetsModelMirror? {
        ...
        return upstreamMirror("models/whisperkit/\(fullName)/legacy-local-v1/manifest.json")
    }
...
public enum ManagedASRModelDownloader {
    ...
    public static func downloadIfNeeded(
        ...
        mirrorResolver: MeetsModelMirrorManifestResolver = .shared,
    ...
    private static func performDownload(
        ...
        mirrorResolver: MeetsModelMirrorManifestResolver,
        ...
        if let mirror = plan.mirror {
            do {
                report("Checking Meets model mirror...")
                ...
                report("Meets mirror unavailable; trying Hugging Face...")
            }
        } else {
    ...
    private static func moveCacheAside(_ cacheDirectory: URL) throws -> URL? {
        ...
        let backup = cacheDirectory.deletingLastPathComponent().appendingPathComponent(
            ".\(cacheDirectory.lastPathComponent).meets-mirror-fallback-\(UUID().uuidString)",
            isDirectory: true
        )
    ...
    private static func recoverInterruptedFallbackCache(for plan: ManagedASRModelPlan) throws {
        ...
        let prefix = ".\(cacheDirectory.lastPathComponent).meets-mirror-fallback-"
