...
public struct MeetsModelMirror: Hashable, Sendable {
...
public final class MeetsModelMirrorManifestResolver: @unchecked Sendable {
    ...
    public func resolve(
        ...
        guard (200..<300).contains(http.statusCode) else {
            throw MeetsModelMirrorManifestError.invalidHTTPStatus(http.statusCode, mirror.manifestURL)
        }
        ...
        guard payload.modelID == modelID else {
            throw MeetsModelMirrorManifestError.unexpectedModelID(
                expected: modelID,
                actual: payload.modelID
            )
        }
        let version = payload.version?.trimmingCharacters(in: .whitespacesAndNewlines) ?? ""
        guard !version.isEmpty else { throw MeetsModelMirrorManifestError.invalidVersion }
        ...
        let files = try payload.files.map { entry -> ModelDownloadFile in
            guard Self.isSafeRelativePath(entry.relativePath),
                ...
                throw MeetsModelMirrorManifestError.invalidFile(entry.relativePath)
            }
            guard seenPaths.insert(entry.relativePath).inserted else {
                throw MeetsModelMirrorManifestError.duplicatePath(entry.relativePath)
            }
        ...
        guard !files.isEmpty else { throw MeetsModelMirrorManifestError.invalidManifest(mirror.manifestURL) }
