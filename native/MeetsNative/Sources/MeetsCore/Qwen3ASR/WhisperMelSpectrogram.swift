...
/// - hop_length: 160
/// - n_mels: 128 (from `MeetsQwen3AsrConfig.numMelBins`)
...
public final class MeetsQwen3WhisperMelSpectrogram {

    // MARK: Config

    /// Window size and DFT length (Whisper-specific, not in MeetsQwen3AsrConfig).
    private let nFFT: Int = 400
    /// Hop length between frames (Whisper-specific, not in MeetsQwen3AsrConfig).
    private let hopLength: Int = 160
    /// Number of mel bins (matches `MeetsQwen3AsrConfig.numMelBins`).
    private let nMels: Int = MeetsQwen3AsrConfig.numMelBins
    /// Sample rate (matches `MeetsQwen3AsrConfig.sampleRate`).
    private let sampleRate: Int = MeetsQwen3AsrConfig.sampleRate
    ...
    ///   Suitable for `MeetsQwen3AsrManager.transcribe(melSpectrogram:)`.
