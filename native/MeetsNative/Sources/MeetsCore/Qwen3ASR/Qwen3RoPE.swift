...
public struct MeetsQwen3RoPE: Sendable {
    ...
    public init() {
        self.headDim = MeetsQwen3AsrConfig.headDim
        self.maxPosition = MeetsQwen3AsrConfig.maxCacheSeqLen
        let theta = Float(MeetsQwen3AsrConfig.ropeTheta)
        let dim = Float(MeetsQwen3AsrConfig.headDim)

        var freq = [Float](repeating: 0.0, count: MeetsQwen3AsrConfig.headDim / 2)
        for i in stride(from: 0, to: MeetsQwen3AsrConfig.headDim, by: 2) {
        ...
        let halfDim = MeetsQwen3AsrConfig.headDim / 2
        var cosVals = [Float](repeating: 0.0, count: maxPosition * MeetsQwen3AsrConfig.headDim)
        var sinVals = [Float](repeating: 0.0, count: maxPosition * MeetsQwen3AsrConfig.headDim)

        for p in 0..<maxPosition {
            let pos = Float(p)
            let offset = p * MeetsQwen3AsrConfig.headDim
            for i in 0..<halfDim {
