...
private enum MeetsQwen3ModelFiles {
...
private let logger = Logger(subsystem: "FluidAudio", category: "MeetsQwen3AsrModels")

/// Qwen3-ASR model variant (precision).
public enum MeetsQwen3AsrVariant: String, CaseIterable, Sendable {
...
public struct MeetsQwen3AsrModels: Sendable {
    public let audioEncoder: MLModel
    public let decoderStateful: MLModel
    public let embeddingWeights: MeetsQwen3EmbeddingWeights
    ...
    public static func load(
    ...
    ) async throws -> MeetsQwen3AsrModels {
        ...
        let audioEncoder = try await loadModel(
            named: MeetsQwen3ModelFiles.audioEncoderName,
            from: directory,
        ...
        let decoderStateful = try await loadModel(
            named: MeetsQwen3ModelFiles.decoderStatefulName,
            from: directory,
            configuration: modelConfig
        )

        // Load embedding weights for Swift-side lookup
        let embeddingWeights = try loadMeetsQwen3EmbeddingWeights(from: directory)
    ...
    public static func defaultCacheDirectory(variant: MeetsQwen3AsrVariant = .f32) -> URL {
        modelsRootDirectory()
    ...
    private static func loadModel(
        ...
        if FileManager.default.fileExists(atPath: compiledPath.path) {
            ...
            throw MeetsQwen3AsrError.modelNotFound(name)
        }
    ...
    private static func loadMeetsQwen3EmbeddingWeights(from directory: URL) throws -> MeetsQwen3EmbeddingWeights {
        #if !arch(arm64)
        // Fail loading with a clear error instead of reaching the fatalError in
        // embedding(for:) on non-Apple-Silicon machines.
        throw MeetsQwen3AsrError.unsupportedHardware
        #endif
        let path = directory.appendingPathComponent(MeetsQwen3ModelFiles.embeddings)
        guard FileManager.default.fileExists(atPath: path.path) else {
            throw MeetsQwen3AsrError.modelNotFound("qwen3_asr_embeddings.bin")
        }

        let start = CFAbsoluteTimeGetCurrent()
        let weights = try MeetsQwen3EmbeddingWeights(contentsOf: path)
        let elapsed = CFAbsoluteTimeGetCurrent() - start
        logger.info(
    ...
    private static func loadVocabulary(from directory: URL) throws -> [Int: String] {
        let vocabPath = directory.appendingPathComponent(MeetsQwen3ModelFiles.vocab)
        guard FileManager.default.fileExists(atPath: vocabPath.path) else {
            throw MeetsQwen3AsrError.modelNotFound(MeetsQwen3ModelFiles.vocab)
        }

        let data = try Data(contentsOf: vocabPath)
        guard let stringToId = try JSONSerialization.jsonObject(with: data) as? [String: Int] else {
            throw MeetsQwen3AsrError.invalidVocabulary
        }
...
public final class MeetsQwen3EmbeddingWeights: Sendable {
    ...
    public init(contentsOf url: URL) throws {
        ...
        guard vocabSize == MeetsQwen3AsrConfig.vocabSize else {
            throw MeetsQwen3AsrError.generationFailed(
                "Embedding vocab size \(vocabSize) != config \(MeetsQwen3AsrConfig.vocabSize)"
            )
        }
        guard hiddenSize == MeetsQwen3AsrConfig.hiddenSize else {
            throw MeetsQwen3AsrError.generationFailed(
                "Embedding hidden size \(hiddenSize) != config \(MeetsQwen3AsrConfig.hiddenSize)"
            )
        }
...
public enum MeetsQwen3AsrError: Error, LocalizedError {
