...
// Original: https://github.com/FluidInference/FluidAudio — types renamed with MeetsQwen3 prefix.
...
private let logger = Logger(subsystem: "FluidAudio", category: "MeetsQwen3AsrManager")
...
public actor MeetsQwen3AsrManager {
    private var models: MeetsQwen3AsrModels?
    private let rope: MeetsQwen3RoPE
    private let melExtractor: MeetsQwen3WhisperMelSpectrogram

    public init() {
        self.rope = MeetsQwen3RoPE()
        self.melExtractor = MeetsQwen3WhisperMelSpectrogram()
    }

    /// Load all models from the specified directory.
    public func loadModels(from directory: URL, computeUnits: MLComputeUnits = .all) async throws {
        models = try await MeetsQwen3AsrModels.load(from: directory, computeUnits: computeUnits)
    ...
    public func transcribe(
        ...
        let maxSamples = Int(MeetsQwen3AsrConfig.maxAudioSeconds * 16_000)
        guard audioSamples.count <= maxSamples else {
            throw MeetsQwen3AsrError.audioTooLong(
                "Audio is \(String(format: "%.1f", Double(audioSamples.count) / 16_000.0))s long; Qwen3-ASR supports up to \(Int(MeetsQwen3AsrConfig.maxAudioSeconds))s."
            )
        }
        let mel = melExtractor.compute(audio: audioSamples)
        guard !mel.isEmpty else {
            throw MeetsQwen3AsrError.generationFailed("Audio too short to extract mel spectrogram")
        }
    ...
    public func transcribe(
        audioSamples: [Float],
        language: MeetsQwen3AsrConfig.Language?,
        maxNewTokens: Int = 512
    ) async throws -> String {
    ...
    public func transcribe(
        ...
        guard let models = models else {
            throw MeetsQwen3AsrError.generationFailed("Models not loaded")
        }

        let start = CFAbsoluteTimeGetCurrent()

        // Resolve language
        let resolvedLanguage: MeetsQwen3AsrConfig.Language?
        if let lang = language {
            resolvedLanguage = MeetsQwen3AsrConfig.Language(from: lang)
    ...
    private func encodeAudio(
        melSpectrogram: [[Float]],
        models: MeetsQwen3AsrModels
    ) throws -> [[Float]] {
        let windowSize = MeetsQwen3AsrConfig.melWindowSize
        ...
        while offset < numFrames {
            ...
            guard let features = prediction.featureValue(for: "audio_features")?.multiArrayValue else {
                throw MeetsQwen3AsrError.encoderFailed("No audio_features output")
            }

            let numOutputFrames: Int
            if currentWindowSize == windowSize {
                numOutputFrames = MeetsQwen3AsrConfig.outputFramesPerWindow
            } else {
                numOutputFrames =
                    (currentWindowSize + MeetsQwen3AsrConfig.convDownsampleFactor - 1)
                    / MeetsQwen3AsrConfig.convDownsampleFactor
            }

            for f in 0..<numOutputFrames {
                var vec = [Float](repeating: 0.0, count: MeetsQwen3AsrConfig.encoderOutputDim)
                for d in 0..<MeetsQwen3AsrConfig.encoderOutputDim {
                    let idx = f * MeetsQwen3AsrConfig.encoderOutputDim + d
    ...
    private func createMelInput(
        ...
        let shape: [NSNumber] = [1, NSNumber(value: MeetsQwen3AsrConfig.numMelBins), NSNumber(value: padTo)]
        let array = try MLMultiArray(shape: shape, dataType: .float32)
        let ptr = array.dataPointer.bindMemory(to: Float.self, capacity: array.count)

        ptr.initialize(repeating: 0.0, count: array.count)

        for bin in 0..<MeetsQwen3AsrConfig.numMelBins {
    ...
    private static let taskTokens: [MeetsQwen3AsrConfig.Language: [Int32]] = [
    ...
    private func buildPromptTokens(numAudioFrames: Int, language: MeetsQwen3AsrConfig.Language?) -> [Int32] {
        var tokens: [Int32] = []

        // System message with optional task description
        tokens.append(Int32(MeetsQwen3AsrConfig.imStartTokenId))
        tokens.append(Int32(MeetsQwen3AsrConfig.systemTokenId))
        tokens.append(Int32(MeetsQwen3AsrConfig.newlineTokenId))
        if let lang = language, let taskToks = Self.taskTokens[lang] {
            tokens.append(contentsOf: taskToks)
        }
        tokens.append(Int32(MeetsQwen3AsrConfig.imEndTokenId))
        tokens.append(Int32(MeetsQwen3AsrConfig.newlineTokenId))

        // User message with audio
        tokens.append(Int32(MeetsQwen3AsrConfig.imStartTokenId))
        tokens.append(Int32(MeetsQwen3AsrConfig.userTokenId))
        tokens.append(Int32(MeetsQwen3AsrConfig.newlineTokenId))
        tokens.append(Int32(MeetsQwen3AsrConfig.audioStartTokenId))
        for _ in 0..<numAudioFrames {
            tokens.append(Int32(MeetsQwen3AsrConfig.audioTokenId))
        }
        tokens.append(Int32(MeetsQwen3AsrConfig.audioEndTokenId))
        tokens.append(Int32(MeetsQwen3AsrConfig.imEndTokenId))
        tokens.append(Int32(MeetsQwen3AsrConfig.newlineTokenId))

        // Assistant start
        tokens.append(Int32(MeetsQwen3AsrConfig.imStartTokenId))
        tokens.append(Int32(MeetsQwen3AsrConfig.assistantTokenId))
        tokens.append(Int32(MeetsQwen3AsrConfig.newlineTokenId))
    ...
    private func embedAndMerge(
        ...
        for i in 0..<promptTokens.count {
            if promptTokens[i] == Int32(MeetsQwen3AsrConfig.audioTokenId), audioIdx < audioFeatures.count {
    ...
    private func generate(
        ...
        guard promptLength > 0 else {
            throw MeetsQwen3AsrError.generationFailed("Empty prompt")
        }

        let effectiveMaxNew = min(maxNewTokens, MeetsQwen3AsrConfig.maxCacheSeqLen - promptLength)
        guard effectiveMaxNew > 0 else {
            throw MeetsQwen3AsrError.generationFailed(
                "Prompt length \(promptLength) exceeds cache capacity \(MeetsQwen3AsrConfig.maxCacheSeqLen)"
            )
        }
        ...
        let decHiddenArray = try MLMultiArray(
            shape: [1, 1, NSNumber(value: MeetsQwen3AsrConfig.hiddenSize)], dataType: .float32
        )
        let decHiddenPtr = decHiddenArray.dataPointer.bindMemory(
            to: Float.self, capacity: MeetsQwen3AsrConfig.hiddenSize
        )
        let decodeCosArray = try MLMultiArray(
            shape: [1, 1, NSNumber(value: MeetsQwen3AsrConfig.headDim)], dataType: .float32
        )
        let decodeCosPtr = decodeCosArray.dataPointer.bindMemory(
            to: Float.self, capacity: MeetsQwen3AsrConfig.headDim
        )
        ...
        if !MeetsQwen3AsrConfig.eosTokenIds.contains(firstTokenId) {
        ...
        for _ in 1..<effectiveMaxNew {
            ...
            nextEmbedding.withUnsafeBufferPointer { src in
                _ = memcpy(decHiddenPtr, src.baseAddress!, MeetsQwen3AsrConfig.hiddenSize * MemoryLayout<Float>.size)
            }
            ...
            if MeetsQwen3AsrConfig.eosTokenIds.contains(tokenId) {
                break
            }
    ...
    private func runStatefulDecoder(
        ...
        guard let logits = output.featureValue(for: "logits")?.multiArrayValue else {
            throw MeetsQwen3AsrError.decoderFailed("Missing logits from stateful decoder")
        }
    ...
    private func argmaxFromLogits(_ logits: MLMultiArray) -> Int {
        let ptr = logits.dataPointer.bindMemory(to: Float.self, capacity: MeetsQwen3AsrConfig.vocabSize)
        var maxVal: Float = 0
        var maxIdx: vDSP_Length = 0
        vDSP_maxvi(ptr, 1, &maxVal, &maxIdx, vDSP_Length(MeetsQwen3AsrConfig.vocabSize))
        return Int(maxIdx)
    }
    ...
    private func decodeTokens(_ tokenIds: [Int], vocabulary: [Int: String]) -> String {
        var startIdx = 0
        if let asrIdx = tokenIds.firstIndex(of: MeetsQwen3AsrConfig.asrTextTokenId) {
            startIdx = asrIdx + 1
        }
    ...
    private func createBatchedHiddenArray(embeddings: [[Float]]) throws -> MLMultiArray {
        ...
        let shape: [NSNumber] = [1, NSNumber(value: seqLen), NSNumber(value: MeetsQwen3AsrConfig.hiddenSize)]
        let array = try MLMultiArray(shape: shape, dataType: .float32)
        let totalCount = seqLen * MeetsQwen3AsrConfig.hiddenSize
        let ptr = array.dataPointer.bindMemory(to: Float.self, capacity: totalCount)
        for i in 0..<seqLen {
            let offset = i * MeetsQwen3AsrConfig.hiddenSize
            let emb = embeddings[i]
            for j in 0..<MeetsQwen3AsrConfig.hiddenSize {
                ptr[offset + j] = emb[j]
            }
        }
        return array
    ...
    private func createBatchedPositionArray(values: [Float], seqLen: Int) throws -> MLMultiArray {
        let shape: [NSNumber] = [1, NSNumber(value: seqLen), NSNumber(value: MeetsQwen3AsrConfig.headDim)]
        let array = try MLMultiArray(shape: shape, dataType: .float32)
